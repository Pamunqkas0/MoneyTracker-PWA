...
export function GoalDetailDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showHandle className="p-0 overflow-hidden rounded-t-[32px] md:rounded-[36px] bg-[#F7F4EE] border border-black/[0.04] shadow-2xl max-w-lg w-full mx-auto max-h-[92vh] flex flex-col">
        {/* Top Floating Close Button */}
        <div className="absolute top-4 right-4 z-20">
          <button
            type="button"
            onClick={() => onOpenChange(false)}
            className="w-8 h-8 rounded-full bg-white/80 backdrop-blur-xs hover:bg-white text-stone-600 flex items-center justify-center transition-all shadow-xs cursor-pointer active:scale-95"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto custom-scrollbar p-5 sm:p-7 flex flex-col gap-4">
          ...
          <div className="bg-[#F7F4EE] rounded-3xl p-4 sm:p-6 flex flex-col items-center text-center relative overflow-hidden">
            ...
            <div className="w-32 h-32 sm:w-36 sm:h-36 rounded-full bg-[#FAD170]/90 flex items-center justify-center relative shadow-inner my-2 border-4 border-white/60">
            ...
            <div className="inline-flex items-center gap-1.5 bg-[#FDD5C1] text-stone-900 font-extrabold text-sm sm:text-base px-5 py-1.5 rounded-full mt-2.5 shadow-2xs">
            ...
            <div className="w-full max-w-xs mt-4 flex flex-col gap-1.5">
              ...
              <div className="w-full bg-black/5 h-2.5 rounded-full overflow-hidden p-0.5">
                <div
                  className="bg-[#1A1A1A] h-full rounded-full transition-all duration-500"
                  style={{ width: `${pct}%` }}
                />
              </div>
              <div className="flex items-center justify-center gap-1 text-[11px] font-medium text-stone-500 mt-1">
          ...
          <div className="grid grid-cols-5 gap-2 w-full">
            ...
            <button
              ...
              className={cn(
                "rounded-2xl p-2.5 sm:p-3 flex flex-col items-center justify-center gap-1.5 border border-black/[0.03] shadow-xs hover:bg-stone-50 active:scale-95 transition-all text-[10px] font-semibold text-stone-700 cursor-pointer",
                activeAction === "stats" ? "bg-[#1A1A1A] text-white" : "bg-white"
              )}
            >
              <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center shadow-2xs", activeAction === "stats" ? "bg-white/20 text-white" : "bg-[#DAEFEA] text-teal-800")}>
            ...
            <button
              ...
              className={cn(
                "rounded-2xl p-2.5 sm:p-3 flex flex-col items-center justify-center gap-1.5 border border-black/[0.03] shadow-xs hover:bg-stone-50 active:scale-95 transition-all text-[10px] font-semibold text-stone-700 cursor-pointer",
                activeAction === "topup" ? "bg-[#1A1A1A] text-white" : "bg-white"
              )}
            >
              <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center shadow-2xs", activeAction === "topup" ? "bg-white/20 text-white" : "bg-[#D8F5A2] text-emerald-800")}>
            ...
            <button
              ...
              className={cn(
                "rounded-2xl p-2.5 sm:p-3 flex flex-col items-center justify-center gap-1.5 border border-black/[0.03] shadow-xs hover:bg-stone-50 active:scale-95 transition-all text-[10px] font-semibold text-stone-700 cursor-pointer",
                activeAction === "withdraw" ? "bg-[#1A1A1A] text-white" : "bg-white"
              )}
            >
              <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center shadow-2xs", activeAction === "withdraw" ? "bg-white/20 text-white" : "bg-[#FDD5C1] text-[#E85024]")}>
            ...
            <button
              ...
              className="bg-white rounded-2xl p-2.5 sm:p-3 flex flex-col items-center justify-center gap-1.5 border border-black/[0.03] shadow-xs hover:bg-stone-50 active:scale-95 transition-all text-[10px] font-semibold text-stone-700 cursor-pointer"
            >
              <div className="w-8 h-8 rounded-xl bg-[#E0E6FD] text-indigo-700 flex items-center justify-center shadow-2xs">
            ...
            <button
              ...
              className="bg-white rounded-2xl p-2.5 sm:p-3 flex flex-col items-center justify-center gap-1.5 border border-black/[0.03] shadow-xs hover:bg-stone-50 active:scale-95 transition-all text-[10px] font-semibold text-stone-700 cursor-pointer"
            >
              <div className="w-8 h-8 rounded-xl bg-surface-muted/90 text-stone-700 flex items-center justify-center shadow-2xs">
          ...
          {activeAction !== "none" && activeAction !== "stats" && (
            <div className="bg-white rounded-3xl p-4 sm:p-5 border border-black/[0.04] shadow-xs flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-[#18181B] flex items-center gap-1.5">
                  {activeAction === "topup" ? (
                    ...
                    <>
                      <TrendingDown className="w-4 h-4 text-[#E85024]" />
              ...
              <div className="flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    className="pl-10 pr-4 h-11 rounded-2xl bg-surface-muted/60 border-stone-200/60 font-bold text-sm"
                  />
                </div>
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    activeAction === "topup" ? "bg-[#E85024] hover:bg-[#d44319]" : "bg-[#1A1A1A] hover:bg-black"
                  )}
                >
          ...
          <div className="flex flex-col gap-2.5 pt-1">
            ...
            <div className="flex flex-col gap-2">
              {activities.map((act) => (
                <div
                  key={act.id}
                  className="bg-white rounded-2xl p-3.5 flex items-center justify-between border border-black/[0.03] shadow-xs hover:-translate-y-0.5 transition-all"
                >
                  {/* Sisi Kiri: Squircle Soft Peach + Stack Info */}
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-2xl bg-[#FDD5C1]/70 flex items-center justify-center text-sm shadow-2xs shrink-0">
                      <AnimatedEmoji emoji={goal.emoji || "🎯"} size={18} />
                    </div>

                    <div className="min-w-0">
                      <h4 className="truncate text-xs font-bold text-[#18181B] leading-tight">
                        {act.name}
                      </h4>
                      <p className="font-extrabold text-xs text-emerald-600 tabular-nums mt-0.5">
